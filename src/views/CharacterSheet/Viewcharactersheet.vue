<template>
    <h1 class="display-6 text-light text-center mt-5">Wybierz karte postaci, którą chcesz przejrzeć:</h1>
    ...
    <div style="color: white;" class="container mt-4">
        <table class="table custom-table table-dark mx-auto">
            ...
            <tbody v-if="this.items.length > 0">
                    <tr v-for="item in VisiblePost" :key="item.characterId" class="position-relative">
                        <td scope="row">
                            <router-link class="stretched-link" style="text-decoration: none; color:whitesmoke" 
                            :to="{ name: 'ByCharacterId', params: { ID: item.characterId, link: '/Kartypostaci' }}">
                                {{ item.characterName }}
                            </router-link>
                        </td> 
                    </tr>
    ...
    computed: {
        filteredItems() {
            if (this.searchFilter !== '') {
                ...
                item.characterName.toLowerCase().includes(this.searchFilter.toLowerCase()));
            }
