...
<script>
   ...
   data() {
    ...
    computed: {
        filterednull() {
            return this.ar = this.items.filter((item) => item !== null);
        },
        filteredItems() {
            if (this.searchFilter !== '') {
                return this.filterednull.filter(item => 
                item.className.toLowerCase().includes(this.searchFilter.toLowerCase()));
            }

            return this.filterednull;
        },
        VisiblePost() {
    ...
    mounted() {
        axios.get('https://kumpleismokibbkservice.azurewebsites.net/api/DndClass/upvoted', {
                headers: {
...
methods: {
    sortLowest() {
        this.items = this.ar.sort((a,b) => a.upvotes > b.upvotes ? 1 : -1)
    },
    sortHighest() {
        this.items = this.ar.sort((a,b) => a.upvotes < b.upvotes ? 1 : -1)
    },
